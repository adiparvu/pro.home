            ...
            VStack(spacing: 0) {
                ...
                if !service.contractors.isEmpty {
                    HStack(spacing: 10) {
                        ...
                        TextField("Search…", text: $search).font(.system(size: 15)).foregroundStyle(.white).tint(.blue)
                    }
                ...
                if service.isLoading {
                    ...
                    VStack(spacing: 14) {
                        Spacer()
                        Image(systemName: "person.badge.key.fill").font(.system(size: 44)).foregroundStyle(.white.opacity(0.18))
                        Text(service.contractors.isEmpty ? "No contractors yet" : "No results").font(.system(size: 17)).foregroundStyle(.white.opacity(0.5))
                        if service.contractors.isEmpty {
        ...
        GlassCard {
            HStack(spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    Text(contractor.name).font(.system(size: 15, weight: .semibold)).foregroundStyle(.white)
                    Text(contractor.specialty.capitalized).font(.system(size: 12)).foregroundStyle(.white.opacity(0.45))
                }
                Spacer()
