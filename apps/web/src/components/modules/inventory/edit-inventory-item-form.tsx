...
const schema = z.object({
  ...
  current_value: z.coerce.number().min(0).optional().or(z.literal('')),
...
export function EditInventoryItemForm({ item, rooms = [] }: EditInventoryItemFormProps) {
  const router = useRouter()
  const [serverError, setServerError] = React.useState<string | null>(null)
  const [photoUrls, setPhotoUrls] = React.useState<string[]>(item.photo_urls ?? [])
  ...
  async function handlePhotoUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const path = `inventory-photos/${item.id}/${Date.now()}-${file.name.replace(/[^a-z0-9.-]/gi, '_')}`
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          >
            {uploadingPhoto ? <span className="text-xs">…</span> : <Camera className="h-5 w-5" />}
          </button>
        </div>
      ...
      <div className="flex gap-3 pt-2">
        ...
        <Button type="submit" size="lg" fullWidth loading={isSubmitting}>Save Changes</Button>
      </div>
    </form>
  )
}
