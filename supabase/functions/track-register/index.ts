...
// so the aggregator starts pushing status events to `track-webhook`. Called by
...
//   3. Store trackerId + courier on the package (service role).
//
...
serve(async (req) => {
  ...
  // Authenticate the caller and confirm they can see this package.
  ...
  // Register the tracker with Ship24.
  ...
  // Persist the tracker id (service role bypasses RLS; access already checked).
